        ...
        with torch.no_grad():
            for j, eval_data in enumerate(loader):
                ...
                if groupwise:
                    ...
                    if (args.model == 'resaligned' and args.method != 'scratch'):
                        loss = self.criterion(outputs[-1], labels) + self.criterion(outputs[-2],
                                                                                    labels) + self.criterion(
                            outputs[-3], labels) + self.criterion(outputs[-4], labels)
                    else:
                        loss = self.criterion(outputs[-1], labels)

                    eval_loss += loss.item() * len(labels)
                    preds = torch.argmax(outputs[-1], 1)
                    acc = (preds == labels).float().squeeze()
                    eval_acc += acc.sum()
    ...
    def compute_confusion_matix(self, dataset='test', num_classes=2,
        ...
        with torch.no_grad():
            for i, data in enumerate(dataloader):
                ...
                outputs = self.model(inputs, get_inter=True)
                if self.get_inter:
                ...
                output_set = torch.cat((output_set, outputs[-1].cpu()))
                if self.get_inter:
