...
def main():
    ...
    if args.modelpath is not None:
        outputs = model(torch.rand([1,3,176,176]).cuda(device=args.device), get_inter = True)
        layer_list = []
    ...
    if (args.method.startswith('kd') or args.teacher_path is not None) and args.mode != 'eval':
        ...
        if args.model == 'resaligned':
            outputs = teacher(torch.rand([1,3,176,176]), get_inter = True)
            layer_list = []
            teacher_feature_size = outputs[0].size(1)
            student_feature_size = outputs[0].size(1)
            layer_list.append(nn.Linear(student_feature_size, teacher_feature_size))
            layer_list.append(nn.Linear(student_feature_size, teacher_feature_size))
            layer_list.append(nn.Linear(student_feature_size, teacher_feature_size))
            teacher.adaptation_layers = nn.ModuleList(layer_list)
            teacher.adaptation_layers.cuda()
