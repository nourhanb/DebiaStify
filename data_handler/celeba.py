...
class CelebA(VisionDataset):
    ...
    def __init__(self, root, split="train", target_type="attr", transform=None,
        ...
        self.cluster_attr = 'cluster_label'
        ...
        attr_index = attr.reset_index()
        grp = pandas.read_csv(fn("clusters/Wearing_Necklace_layer2_k8.csv"))
        grp = grp[['image_ID','cluster_label']]
        attr_index = attr_index.rename(columns={"index":"image_ID"})
        attr_index = attr_index.merge(grp, on='image_ID', how='left')
        attr_index.fillna({'cluster_label':-4}, inplace=True)
        attr_index['cluster_label'] = attr_index['cluster_label'].astype(int)
        attr_index.set_index('image_ID', inplace=True, drop=True)
        attr = attr_index
        ...
        self.cluster_idx = self.attr_names.index(self.cluster_attr)
    ...
    def __getitem__(self, index):
        ...
        cluster = self.attr[index, self.cluster_idx]
        ...
        if self.split =='train':
            return X, feature, cluster, target, (sensitive, img_name)
        else:
            return X, feature, sensitive, target, (cluster, img_name)
